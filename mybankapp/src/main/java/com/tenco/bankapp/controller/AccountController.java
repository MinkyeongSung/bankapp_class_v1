...
public class AccountController {
	...
	public String depositProc(DepositFormDto dto) {
		...
		if (dto.getAmount() == null) {
			throw new UnAuthorizedException("금액을 입력해주세요", HttpStatus.BAD_REQUEST);
		}
	...
	public String transferProc(TransferFormDto dto) {
		...
		if (dto.getAmount() == null) {
			throw new UnAuthorizedException("이체 금액을 입력하시오", HttpStatus.BAD_REQUEST);
		}

		if (dto.getAmount().longValue() <= 0) {
			throw new UnAuthorizedException("이체 금액이 0원 이하일 수 없습니다", HttpStatus.BAD_REQUEST);
		}

		if (dto.getWAccountNumber() == null || dto.getWAccountNumber().isEmpty()) {
			throw new UnAuthorizedException("출금 계좌 번호를 입력하시오", HttpStatus.BAD_REQUEST);
		}

		if (dto.getDAccountNumber() == null || dto.getWAccountNumber().isEmpty()) {
			throw new UnAuthorizedException("이체 계좌 번호를 입력하시오", HttpStatus.BAD_REQUEST);
		}

		if (dto.getPassword() == null || dto.getPassword().isEmpty()) {
			throw new UnAuthorizedException("출금 계좌 비밀번호를 입력하시오", HttpStatus.BAD_REQUEST);
		}
